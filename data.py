...
def create_data_loader(X, y1, y2, shuffle, batch_size):
    data_set = TensorDataset(torch.from_numpy(X).long(), torch.from_numpy(np.array(y1)).long(), torch.from_numpy(np.array(y2)).long())
